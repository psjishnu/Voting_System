...
sql=sql.replace("x1",loggedinname);sql=sql.replace("x2",nn);
function verify(x){
  ins(x);
 }
con.query("SELECT name,v FROM vote", function (err, result,fields ) {
...
function ins(ax){
  if(ax==0){
    ...
    jis.addBlock(new Block(nn,p,loggedinname));
      res.render('welcome',{welname:loggedinname , welcomeins:"VOTED SUCCESSFULLY"});
  }
  else{
      res.render('welcome',{welname:loggedinname,welcomeins:"ALREADY VOTED"}); }
}
});
...
app.post('/thank', urlencodedParser, function (req, res){
  ...
  if(nn==adname && np==adpass){
    ...
    if(len1<2){
      ...
      res.render('admin',{adminmsg:"",contacts1:arr,contacts:(obj.chain)});
    }
  }
...
app.get('/rollback',urlencodedParser,function(req,res){
  ...
  res.render('admin',{adminmsg:"CORRECTED",contacts1:arr,contacts:(obj.chain)});

}
  else{
    res.render('admin',{adminmsg:"DATABASE EMPTY",contacts1:arr,contacts:(obj.chain)});
  }
});
